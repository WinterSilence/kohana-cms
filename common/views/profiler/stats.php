		...
		<tr class="mark time">
			...
			<td class="<?php echo $key ?>">
				<div>
					...
					<?php if ($key === 'total'): ?>
						<div class="graph" style="left: <?php echo max(0, 100 - $stats[$key]['time'] / max(0.01, $group_stats[$group]['max']['time']) * 100) ?>%"></div>
					<?php endif ?>
				</div>
			</td>
			<?php endforeach ?>
		...
		<tr class="mark memory">
			...
			<td class="<?php echo $key ?>">
				<div>
					...
					<?php if ($key === 'total'): ?>
						<div class="graph" style="left: <?php echo max(0, 100 - $stats[$key]['memory'] / max(0.01, $group_stats[$group]['max']['memory']) * 100) ?>%"></div>
					<?php endif ?>
				</div>
			</td>
			<?php endforeach ?>
